import {
  ...
  lineBreakG,
  defaultOptions,
} from '../../../acorn/package/dist/acorn.mjs.js';
...
LooseParser.prototype.closes = function closes(closeTok, indent, line, blockHeuristic) {
  ...
  return (
    ...
    (!blockHeuristic || this.nextLineStart >= this.input.length || this.indentationAfter(this.nextLineStart) < indent)
  );
};
...
lp.readToken = function() {
  for (;;) {
    try {
      ...
      this.resetTo(pos);
      if (replace === true) {
        replace = { start: pos, end: pos, type: tokTypes.name, value: dummyValue };
      }
      if (replace) {
        if (this.options.locations) {
...
lp.resetTo = function(pos) {
  ...
  this.toks.exprAllowed =
    !ch ||
    /[[{(,;:?/*=+\-~!|&%^<>]/.test(ch) ||
    (/[enwfd]/.test(ch) && /\b(case|else|return|throw|new|in|(instance|type)?of|delete|void)$/.test(this.input.slice(pos - 10, pos)));
  ...
  while (!this.closes(tokTypes.braceR, indent, line)) {
    ...
    if (
      ...
      if (
        ...
        !isGenerator &&
        !isAsync &&
        ((method.key.type === 'Identifier' && method.key.name === 'constructor') || (method.key.type === 'Literal' && method.key.value === 'constructor'))
      ) {
...
lp$2.parseSubscripts = function(base, start, noCalls, startIndent, line) {
  ...
  for (;;) {
    ...
    if (
      (optional && this.tok.type !== tokTypes.parenL && this.tok.type !== tokTypes.bracketL && this.tok.type !== tokTypes.backQuote) ||
