...
export class TimelineHistoryManager {
    constructor() {
        ...
        this._action =
            /** @type {!UI.ActionRegistration.Action} */ (UI.ActionRegistry.ActionRegistry.instance().action('timeline.show-history'));
    ...
    async showHistoryDropDown() {
        ...
        const model = await DropDown.show(this._recordings, /** @type {!PerformanceModel} */ (this._lastActiveModel), this._button.element);
        if (!model) {
            return null;
        }
...
export class ToolbarButton extends UI.Toolbar.ToolbarItem {
    ...
    constructor(action) {
        ...
        action.addEventListener(UI.ActionRegistration.Events.Enabled, event => this.setEnabled(/** @type {boolean} */ (event.data)));
