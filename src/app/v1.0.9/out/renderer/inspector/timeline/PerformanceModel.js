...
export class PerformanceModel extends Common.ObjectWrapper.ObjectWrapper {
    ...
    setTracingModel(model) {
        ...
        for (const entry of this._extensionTracingModels) {
            entry.model.adjustTime(this._tracingModel.minimumRecordTime() + (entry.timeOffset / 1000) -
    ...
    addExtensionEvents(title, model, timeOffset) {
        ...
        model.adjustTime(this._tracingModel.minimumRecordTime() + (timeOffset / 1000) - /** @type {number} */ (this._recordStartTime));
