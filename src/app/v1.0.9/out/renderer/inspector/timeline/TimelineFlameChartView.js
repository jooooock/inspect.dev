...
class MainSplitWidget extends UI.SplitWidget.SplitWidget {
    ...
    setModelAndUpdateBoundaries(model) {
        ...
        const events = timelineModel.tracks().reduce((prev, curr) => prev.concat(curr.events), /** @type {!Array<!SDK.TracingModel.Event>} */ ([]));
        const minimumBoundary = model.timelineModel().minimumRecordTime();
        const prepareEvents = /** @param {function(!SDK.TracingModel.Event): boolean} filterFunction */ /** @param {function(!SDK.TracingModel.Event): boolean} filterFunction */ filterFunction => events.filter(filterFunction).map(e => e.startTime - minimumBoundary);
