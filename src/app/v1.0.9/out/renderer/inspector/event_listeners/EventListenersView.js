...
import { frameworkEventListeners } from './EventListenersUtils.js'; // eslint-disable-line no-unused-vars
...
export class ObjectEventListenerBar extends UI.TreeOutline.TreeElement {
    ...
    _setTitle(object, linkifier) {
        ...
        if (this._eventListener.isScrollBlockingType() && this._eventListener.canTogglePassive()) {
            ...
            UI.Tooltip.Tooltip.install(passiveButton, Common.UIString.UIString('Toggle whether event listener is passive or blocking'));
        ...
        this.listItemElement.addEventListener('contextmenu', event => {
            ...
            menu.defaultSection().appendItem(ls `Delete event listener`, this._removeListener.bind(this), !this._eventListener.canRemove());
            menu.defaultSection().appendCheckboxItem(ls `Passive`, this._togglePassiveListener.bind(this), this._eventListener.passive(), !this._eventListener.canTogglePassive());
            menu.show();
        });
    }
    _removeListener() {
