    ...
    _translateProfileTree(nodes) {
        ...
        function buildHitCountFromSamples(nodes, samples) {
            ...
            for (let i = 0; i < samples.length; ++i) {
                ++( /** @type {!Protocol.Profiler.ProfileNode} */(nodeByIdMap.get(samples[i])).hitCount);
            }
        }
        ...
        while (sourceNodeStack.length) {
            ...
            parentNodeStack.push.apply(parentNodeStack, sourceNode.children.map(() => /** @type {!CPUProfileNode} */ (parentNode)));
    ...
    forEachFrame(openFrameCallback, closeFrameCallback, startTime, stopTime) {
        ...
        for (sampleIndex = startIndex; sampleIndex < samplesCount; sampleIndex++) {
            ...
            while (prevNode !== node) {
                ...
                const duration = sampleTime - start;
                stackChildrenDuration[stackTop - 1] += duration;
                closeFrameCallback(prevNode.depth, /** @type {!CPUProfileNode} */ (prevNode), start, duration, duration - stackChildrenDuration[stackTop]);
                --stackTop;
        ...
        if (gcParentNode && idToNode.get(prevId) === gcNode) {
            ...
            const duration = sampleTime - start;
            stackChildrenDuration[stackTop - 1] += duration;
            closeFrameCallback(gcParentNode.depth + 1, /** @type {!CPUProfileNode} */ (node), start, duration, duration - stackChildrenDuration[stackTop]);
            --stackTop;
        ...
        for (let node = idToNode.get(prevId); node && node.parent; node = /** @type {!CPUProfileNode} */ (node.parent)) {
            ...
            const duration = sampleTime - start;
            stackChildrenDuration[stackTop - 1] += duration;
            closeFrameCallback(node.depth, /** @type {!CPUProfileNode} */ (node), start, duration, duration - stackChildrenDuration[stackTop]);
            --stackTop;
        }
    }
    /**
