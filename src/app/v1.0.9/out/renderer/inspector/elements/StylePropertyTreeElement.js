...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
    ...
    _processColor(text, valueChild) {
        ...
        const tooltip = this._editable() ? Common.UIString.UIString('Open color picker. %s', shiftClickMessage) : shiftClickMessage;
        ...
        for (const elementSetting of variationSettings) {
            for (const font of matchingFontFaces) {
                ...
                if (elementSetting.value < fontSetting.minValue || elementSetting.value > fontSetting.maxValue) {
                    warnings.push(ls `Value for setting “${elementSetting.tag}” ${elementSetting.value} is outside the supported range [${fontSetting.minValue}, ${fontSetting.maxValue}] for font-family “${font.getFontFamily()}”.`);
                }
            }
        }
        if (!warnings.length) {
