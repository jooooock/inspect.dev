...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
    constructor() {
        ...
        this._dataGrid.addEventListener(DataGrid.DataGrid.Events.SelectedNode, event => this._infoWidget.render(event.data.data));
        ...
        this._dataGrid.sortNodes(DataGrid.SortableDataGrid.SortableDataGrid.NumericComparator.bind(null, 'timestamp'), false);
    ...
    _messageReceived(message, target) {
        if ('id' in message) {
            ...
            if (this._dataGrid.selectedNode === node) {
                const data = 
                /** @type {?{method: string, direction: string, request: ?Object, response: ?Object, timestamp: number}}*/ (node.data);
