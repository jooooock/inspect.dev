...
export class ListWidget extends VBox {
    ...
    _startEditing(item, element, insertionPoint) {
        ...
        this._editor.beginEdit(item, index, element ? Common.UIString.UIString('Save') : Common.UIString.UIString('Add'), this._commitEditing.bind(this), this._stopEditing.bind(this));
    }
    _commitEditing() {
...
export class Editor {
    constructor() {
        ...
        this._cancelButton = createTextButton(Common.UIString.UIString('Cancel'), this._cancelClicked.bind(this), '', true /* primary */, 'mousedown');
    ...
    _validateControls(forceValid) {
        ...
        for (let index = 0; index < this._controls.length; ++index) {
            ...
            const { valid, errorMessage } = this._validators[index].call(null, /** @type {!T} */ (this._item), this._index, input);
