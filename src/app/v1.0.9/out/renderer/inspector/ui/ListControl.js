...
export class ListControl {
    ...
    _replacedItemsInRange(event) {
        const data = 
        /** @type {{index: number, removed: !Array<T>, inserted: number, keepSelectedIndex: (boolean|undefined)}} */ (event.data);
    ...
    _indexAtOffset(offset) {
        ...
        if (this._mode === ListMode.VariousHeightItems) {
            return Math.min(this._model.length - 1, this._variableOffsets.lowerBound(offset, undefined, 0, this._model.length));
        }
    ...
    _select(index, oldItem, oldElement) {
        ...
        this._delegate.selectedItemChanged(oldItem, newItem, /** @type {?HTMLElement} */ (oldElement), /** @type {?HTMLElement} */ (newElement));
