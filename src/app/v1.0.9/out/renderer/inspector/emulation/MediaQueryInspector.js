...
export class MediaQueryInspector extends UI.Widget.Widget {
    ...
    _onContextMenu(event) {
        ...
        const subMenuItem = contextMenu.defaultSection().appendSubMenuItem(Common.UIString.UIString('Reveal in source code'));
        for (let i = 0; i < contextMenuItems.length; ++i) {
            const title = contextMenuItems[i];
            subMenuItem.defaultSection().appendItem(title, this._revealSourceLocation.bind(this, /** @type {!Workspace.UISourceCode.UILocation} */ (uiLocations.get(title))));
        }
        contextMenu.show();
    }
    /**
    ...
    _createElementFromMediaQueryModel(model) {
        ...
        function appendLabel(marker, expression, atLeft, leftAlign) {
            ...
            marker
                .createChild('div', 'media-inspector-marker-label-container ' + (atLeft ? 'media-inspector-marker-label-container-left' :
