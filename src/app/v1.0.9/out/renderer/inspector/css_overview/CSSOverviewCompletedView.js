...
export class CSSOverviewCompletedView extends UI.Panel.PanelWithSidebar {
    ...
    _onMouseOver(evt) {
        ...
        const node = ( /** @type {!Array<!HTMLElement>} */(evt.composedPath())).find(el => el.dataset && el.dataset.backendNodeId);
        if (!node) {
            return;
        }
    ...
    _contrastIssueToFragment(key, issues) {
      ...
      <button
        title="${ls `Text color ${color} over ${backgroundColor} background results in low contrast for ${issues.length} elements`}"
...
export class ElementNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
    ...
    createCell(columnId) {
        ...
        if (columnId === 'contrastRatio') {
            const cell = this.createTD(columnId);
            const showAPCA = Root.Runtime.experiments.isEnabled('APCA');
            const contrastRatioString = showAPCA ? this.data.contrastRatio.toFixed(2) + '%' : this.data.contrastRatio.toFixed(2);
