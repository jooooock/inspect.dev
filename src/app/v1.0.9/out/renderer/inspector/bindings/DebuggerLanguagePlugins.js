...
class EvalNodeBase extends SDK.RemoteObject.RemoteObjectImpl {
    ...
    constructor(callFrame, sourceType, plugin, object, formatterTag, evalOptions) {
        super(callFrame.debuggerModel.runtimeModel(), object.objectId, object.type, object.subtype, object.value, object.unserializableValue, object.description, object.preview, object.customPreview, object.className);
        this._plugin = plugin;
    ...
    async _createRemoteObject(newObject) {
        ...
        if (!base) {
            return new EvalNodeBase(this._callFrame, this._sourceType, this._plugin, newObject, this.formatterTag, this._evalOptions);
        }
    ...
    async _getEvalBaseFromObject(object) {
        ...
        const baseObject = new EvalNodeBase(this._callFrame, this._sourceType, this._plugin, result, null, this._evalOptions);
...
class EvalNode extends SDK.RemoteObject.RemoteObjectImpl {
    ...
    constructor(callFrame, plugin, sourceType, base, fieldChain, evalOptions) {
        ...
        super(callFrame.debuggerModel.runtimeModel(), 
        /* objectId=*/ undefined, 
        /* type=*/ variableType, 
    ...
    async _expandMember(sourceType, fieldInfo) {
        if (sourceType.typeInfo.hasValue && !sourceType.typeInfo.canExpand && this._base) {
            return EvalNode.evaluate(this._callFrame, this._plugin, sourceType, this._base, this._fieldChain.concat(fieldInfo), this._evalOptions);
        }
        return new EvalNode(this._callFrame, this._plugin, sourceType, this._base, this._fieldChain.concat(fieldInfo), this._evalOptions);
    }
    /**
     * @override
...
class SourceScopeRemoteObject extends SDK.RemoteObject.RemoteObjectImpl {
    ...
    async doGetProperties(ownProperties, accessorPropertiesOnly, generatePreview) {
        ...
        for (const variable of this.variables) {
            let sourceVar;
            try {
                sourceVar = await EvalNode.get(this._callFrame, this._plugin, variable.name, 
                ...
                const sourceFileURLsPromise = (async () => {
                    ...
                    try {
                        ...
                        if (sourceFileURLs.length === 0) {
                            ...
                            console.log(ls `[${plugin.name}] Loaded debug symbols for ${url}, found ${sourceFileURLs.length} source file(s)`);
                        }
