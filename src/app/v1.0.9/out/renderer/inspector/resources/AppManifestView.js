...
export class AppManifestView extends UI.Widget.VBox {
    constructor() {
        ...
        this._emptyView.appendLink('https://developers.google.com/web/fundamentals/engage-and-retain/web-app-manifest/?utm_source=devtools');
    ...
    async _renderManifest(url, data, errors, installabilityErrors, manifestIcons) {
        ...
        if (startURL) {
            ...
            const link = Components.Linkifier.Linkifier.linkifyURL(completeURL, /** @type {!Components.Linkifier.LinkifyURLOptions} */ ({ text: startURL }));
            link.tabIndex = 0;
        ...
        const imageErrors = [];
        const setIconMaskedCheckbox = UI.UIUtils.CheckboxLabel.create(Common.UIString.UIString('Show only the minimum safe area for maskable icons'));
        ...
        const documentationLink = UI.XLink.XLink.create('https://web.dev/maskable-icon/', ls `documentation on maskable icons`);
        this._iconsSection.appendRow().appendChild(UI.UIUtils.formatLocalized('Need help? Read our %s.', [documentationLink]));
        ...
        for (const shortcut of shortcuts) {
            ...
            const link = Components.Linkifier.Linkifier.linkifyURL(shortcutUrl, /** @type {!Components.Linkifier.LinkifyURLOptions} */ ({ text: shortcut.url }));
            link.tabIndex = 0;
        ...
        for (const installabilityError of installabilityErrors) {
            ...
            switch (installabilityError.errorId) {
                ...
                case 'manifest-missing-suitable-icon':
                    ...
                    errorMessage = ls `Manifest does not contain a suitable icon - PNG, SVG or WebP format of at least ${installabilityError.errorArguments[0]
                ...
                case 'no-matching-service-worker':
                    errorMessage = ls `No matching service worker detected. You may need to reload the page, or check that the scope of the service worker for the current page encloses the scope and start URL from the manifest.`;
                    break;
                case 'no-acceptable-icon':
                    ...
                    errorMessage = ls `No supplied icon is at least ${installabilityError.errorArguments[0]
    ...
    async _appendIconResourceToSection(baseUrl, icon, section) {
        ...
        if (!icon.sizes) {
            ...
            const [width, height] = icon.sizes.split('x').map(/** @param {*} x*/ /** @param {*} x*/ x => parseInt(x, 10));
            if (width !== height) {
                iconErrors.push(ls `Icon ${iconUrl} dimensions should be square`);
            }
            else if (image.naturalWidth !== width && image.naturalHeight !== height) {
                iconErrors.push(ls `Actual size (${image.naturalWidth}×${image.naturalHeight})px of icon ${iconUrl} does not match specified size (${width}×${height}px)`);
            }
            else if (image.naturalWidth !== width) {
                iconErrors.push(ls `Actual width (${image.naturalWidth}px) of icon ${iconUrl} does not match specified width (${width}px)`);
            }
            else if (image.naturalHeight !== height) {
                iconErrors.push(ls `Actual height (${image.naturalHeight}px) of icon ${iconUrl} does not match specified height (${height}px)`);
            }
        }
        field.appendChild(wrapper);
        return iconErrors;
