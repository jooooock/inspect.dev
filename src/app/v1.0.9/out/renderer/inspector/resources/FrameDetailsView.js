...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
    ...
    constructor(frame) {
        ...
        if (this._protocolMonitorExperimentEnabled) {
            ...
            this._additionalInfo.setTitle(ls `Additional Information`, ls `This additional (debugging) information is shown because the 'Protocol Monitor' experiment is enabled.`);
    ...
    async doUpdate() {
        ...
        if (!this._frame.unreachableUrl()) {
            ...
            const revealSource = linkifyIcon('mediumicon-sources-panel', ls `Click to reveal in Sources panel`, () => Common.Revealer.reveal(sourceCode));
    ...
    _updateApiAvailability() {
        ...
        if (sabAvailable) {
            ...
            UI.Tooltip.Tooltip.install(this._apiSharedArrayBuffer, sabTransferAvailable ?
    ...
    static maybeAppendLinkToRequest(element, resource) {
        if (resource && resource.request) {
            ...
            const revealRequest = linkifyIcon('mediumicon-network-panel', ls `Click to reveal in Network panel`, () => Network.NetworkPanel.NetworkPanel.selectAndShowRequest(request, Network.NetworkItemView.Tabs.Headers));
    ...
    _maybeAppendLinkForUnreachableUrl() {
        ...
        const revealRequest = linkifyIcon('mediumicon-network-panel', ls `Click to reveal in Network panel (might require page reload)`, () => {
...
async function maybeCreateLinkToElementsPanel(opener) {
    ...
    const linkElement = linkifyIcon('mediumicon-elements-panel', ls `Click to reveal in Elements panel`, () => Common.Revealer.reveal(linkTargetDOMNode));
...
export class OpenedWindowDetailsView extends UI.ThrottledWidget.ThrottledWidget {
    ...
    constructor(targetInfo, isWindowClosed) {
        ...
        UI.Tooltip.Tooltip.install(this._hasDOMAccessValue, ls `Shows whether the opened window is able to access its opener and vice versa`);
        this.update();
    }
    /**
     * @override
