...
export class BackgroundServiceView extends UI.Widget.VBox {
    ...
    constructor(serviceName, model) {
        ...
        this._recordAction =
            /** @type {!UI.ActionRegistration.Action} */ (UI.ActionRegistry.ActionRegistry.instance().action('background-service.toggle-recording'));
        /**
    ...
    async _setupToolbar() {
        ...
        this._toolbar.appendSeparator();
        this._originCheckbox = new UI.Toolbar.ToolbarCheckbox(ls `Show events from other domains`, ls `Show events from other domains`, () => this._refreshView());
    ...
    _createDataGrid() {
        ...
        dataGrid.addEventListener(DataGrid.DataGrid.Events.SelectedNode, event => this._showPreview(/** @type {!EventDataNode} */ (event.data)));
        return dataGrid;
    }
    /**
    ...
    _showPreview(dataNode) {
        ...
        if (this._dataGrid.rootNode().children.length) {
            ...
            centered.createChild('p').appendChild(UI.UIUtils.formatLocalized('Click the record button %s or hit %s to start recording.', [inlineButton, recordKey]));
